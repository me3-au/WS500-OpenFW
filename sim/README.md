...
| `bms_cvl_predisconnect` | **Deliverable #10's two hard preconditions:** a BMS-asserted CVL (3.50 V/cell) below the profile's own CV target (3.60) is honored mid-charge — pack held near 3.50, binding telemetered as `BMS_CVL`; a pre-disconnect warning mid-charge drives the field to exactly zero duty (plant `I_field` confirms it physically) well inside the 1 s check deadline (`CTRL_PREDISC_RAMP_S` = 0.3 s) and holds it there for as long as the warning stays asserted — rotor clamp intact throughout |
