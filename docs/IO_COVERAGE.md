...
|---|---|---|
| Enable / Ignition input (wire 1) | 🔴 pin unknown | trace EXTI/GPIO reads → one of PC13-15/PA0/PA9/PA15/PB3-5/PB14/PC4/PC10 |
| Feature-In input (wire 3) | 🔴 pin unknown | same; multi-function config input |
| Lamp / Feature-Out output (wire 2) | 🔴 pin unknown | trace GPIO BSRR writes / TIM PWM |
| Status LED(s) | 🔴 | trace GPIO output toggles |
...
1. Digital I/O function→pin map (Enable, Feature-In, Feature-Out, LEDs) — needed for a
   complete `board.h` and the bring-up test firmware.
