...
| 10 | CAN Rx for control | **First slice ✅ 2026-07-27**: the §1 dialect-neutral inbound interface (`control/bms_rx.c`) + the CAN-BMS/REC/JK 11-bit frame set (0x351 CVL/CCL/DCL, 0x355 SOC/SOH, 0x356 V/I/T, 0x35A alarms), per-signal staleness with a **declared loss-of-signal fallback** (`CTRL_FAULT_LOST_BMS`, never-seen ≠ went-silent), CCL(A)→W on pack voltage, alarms force the ceiling to 0 W. Wired to `bms_ccl_w` / `soc_pct` / `ext_faults`. 56 host checks. **✅ Both hard preconditions closed 2026-07-28**: **(a)** **CVL consumer** — `ctrl_ceilings_t.bms_cvl_vcell` (V/cell, the one voltage-domain field in an otherwise-Watts struct) is min()'d into the CV target selection in `control.c`'s `ctrl_tick` (NaN-safe, structurally only-ever-lowers, telemetered as `CTRL_BIND_BMS_CVL`); the ONE pack→cell conversion (`ctrl_cell_from_pack`) lives in `Core/Src/main.c` / `sim/sil.c` where `ctrl_ceilings_t` is assembled. **(b)** **pre-disconnect** — decoded from 0x35C's charge-enable bit (byte 0 bit 0, `[SPEC-SIGNOFF]`/bench-pending like every other byte layout in this driver), with a pre-disconnect-then-silence latch mirroring the existing alarm latch; drives an unconditional soft ramp of `field_effort` to zero (`CTRL_PREDISC_RAMP_S` = 0.3 s, `[SPEC-SIGNOFF]`) applied as the LAST step of `ctrl_tick` in every state (BULK/FLOAT/LIMP/run-detect-gated), telemetered as `CTRL_BIND_BMS_PREDISCONNECT`. `control/` core is no longer untouched by this deliverable (by design — see `control.h`/`control.c`). 419 new host checks (`test_bms_rx.c`, new `test_bms_ceilings.c`) + a new SIL scenario (`bms_cvl_predisconnect`, 13 scenarios / 661 checks total in `sim/`). **The pre-disconnect ramp touches the field-drive path and needs its Fable-tier safety review before merge** (repo policy) — not yet performed as of this entry. Still open: a **`bms_required` config flag** — a BMS that is dead at power-on is currently indistinguishable from "no BMS wired" and raises no fault. **Remaining**: that flag, plus Victron DVCC-via-GX, J1939 engine RPM, N2K 127508/127506 ingestion. Vendor frame layouts are `[SPEC-SIGNOFF]`/bench-pending — reconstructed from public CAN-BMS docs, no datasheet or unit to verify against | M5 | 🔨 |
...
- **8.1 SIL — simulated charge cycles against a plant model.** ✅ **BUILT 2026-07-24
  ...
  lying-VBat (clamp plausibility guard), and Ah-revert timing. **Extended 2026-07-28
  (deliverable #10 preconditions): now 13 scenarios / 661 checks** — adds
  `bms_cvl_predisconnect` (BMS CVL lowering the effective CV target mid-charge, honored;
  pre-disconnect mid-charge driving the field to verified-zero duty well inside the 1 s
  check deadline and held there).
